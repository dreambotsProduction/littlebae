...
buttons = [
    [
        InlineKeyboardButton(text=" കമാൻഡ് ഹെല്പ് 💞", callback_data="tiana_"),
    ],
    [
        InlineKeyboardButton(text="വിവരങ്ങൾ ", callback_data="about_"),
        InlineKeyboardButton(text="ഉടമ 👨‍✈️", url=f"https://t.me/{OWNER_USERNAME}"),
    ],
   [
        InlineKeyboardButton(text="📇 അപ്ഡേറ്റ് ചാനൽ ", url=f"http://t.me/{UPDATE_CHANNEL}"),
        InlineKeyboardButton(text="സപ്പോർട്ട് 🫂", url=f"https://t.me/{SUPPORT_CHAT}"),
    ],
    [  
        InlineKeyboardButton(text="➕️എന്നെ ഗ്രൂപ്പിൽ ആഡ് ചെയ്യാൻ ➕️", url=f"https://t.me/{BOT_USERNAME}?startgroup=true"),
    ], 
...
def start(update: Update, context: CallbackContext):
    ...
    if update.effective_chat.type == "private":
        if len(args) >= 1:
          ...
          update.effective_message.reply_text(
            f"ക്ല ക്ല ക്ലി ക്ലി ക്ലൂ ക്ലൂ സ്റ്റാർട്ട്‌ അടിച്ചവൻ ഗ്രൂപ്പിൽ നോക്കി ഗ്രൂപ്പിൽ ഇതാ ഒരു സുന്ദരി ബോട്ട് (ശോ അത് ഞാൻ അല്ലെ ) എനിക്ക് വയ്യ 🙈....",
...
3). Gɪᴠɪɴɢ  Fᴜʟʟ  Pᴇʀᴍɪssɪᴏɴs  Mᴀᴋᴇ  Little Bae  Fᴜʟʟʏ  Usᴇғᴜʟ</b>""",
            parse_mode=ParseMode.HTML,
            reply_markup=InlineKeyboardMarkup(
...
Hᴇʀᴇ  Aʀᴇ  Sᴏᴍᴇ  Essᴇɴᴛɪᴀʟᴛ  Tᴏ  Tʀʏ  Oɴ LittleBae.
...
*Click below Button to Get Source Code.*""",
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                 [
                    InlineKeyboardButton(text="📄 𝙎𝙤𝙪𝙧𝙘𝙚", url="https://t.me/Dream_with_Music"),                 
                    InlineKeyboardButton(text="🔙 𝘽𝙖𝙘𝙠", callback_data="about_"),
                 ]    
                ]
            ),
...
def main():
    ...
    if SUPPORT_CHAT is not None and isinstance(SUPPORT_CHAT, str):
        try:
            dispatcher.bot.sendMessage(f"@{SUPPORT_CHAT}", "ഞാൻ ചത്തെന്നു വിചാരിച്ചോ ഞാൻ അങ്ങനെ ഒന്നും ചാവില്ല മക്കളെ 🌝✌️")
        except Unauthorized:
            LOGGER.warning(
